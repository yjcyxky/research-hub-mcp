...
class AsyncVLLMClient:
    """Async client for communicating with a vLLM server."""
    ...
    def __init__(
        ...
        if AsyncOpenAI is None:
            raise ImportError(
                "AsyncOpenAI client is required. Install it with: pip install openai"
            )
        ...
        self.retry_policy = retry_policy or RetryPolicy()
        self.available_models: List[str] = []
        self._http_client = _build_async_httpx_client(
            base_url=self.base_url, timeout=timeout, pool_size=pool_size
        )
    ...
    async def _execute_with_retry(self, func: Any, description: str) -> Any:
        last_exc: Optional[Exception] = None
        total_attempts = self.retry_policy.attempts + 1
    ...
    async def _get_model_name(self) -> str:
        try:
            models = await self._execute_with_retry(
                lambda: self.client.models.list(), "List models from vLLM"
            )
            if models.data and len(models.data) > 0:
                self.available_models = [
                    m.id for m in models.data if getattr(m, "id", None)
                ]
                model_name = models.data[0].id
                logger.debug("Found model on server: %s", model_name)
                return model_name
            ...
            logger.warning(
                "Could not get model name from server: %s, using 'default'", exc
            )
            return "default"

    async def _ensure_model_name(self) -> str:
        if self.model_name is None:
            self.model_name = await self._get_model_name()
        return self.model_name

    async def _model_or_default(self, model: Optional[str]) -> str:
        model_name = await self._ensure_model_name()
        if model and self.available_models and model not in self.available_models:
            logger.warning(
                "Requested model '%s' not in server models %s; falling back to server default '%s'",
                model,
    ...
    async def generate(
        ...
        model_to_use = await self._model_or_default(model)
        ...
        try:
            ...
            response = await self._execute_with_retry(
                _call, "Call vLLM chat completion"
            )
            ...
            reasoning_text = VLLMClient._stringify_content(
                getattr(message, "reasoning_content", None)
            ).strip()
            content_only = VLLMClient._stringify_content(
                getattr(message, "content", None)
            ).strip()
            content = VLLMClient._combine_message_content(message)
            ...
            logger.error("Error calling vLLM server: %s", exc, exc_info=True)
            raise
...
class AsyncGLiNERClient:
    """Async client for communicating with a GLiNER entity extraction service."""
    ...
    def __init__(
        ...
        self._client = _build_async_httpx_client(
            base_url=self.base_url,
            timeout=timeout,
            pool_size=pool_size,
            headers=headers,
    ...
    async def extract_entities(
        ...
        response = await self._execute_with_retry(
            _call, "Call GLiNER extraction service"
        )
        try:
            parsed = response.json()
        except Exception as exc:  # pragma: no cover - network dependent
            logger.error("GLiNER response is not valid JSON: %s", exc)
            raise
        entities = GLiNERClient._parse_entities(parsed)
        logger.info("GLiNER service returned %d entities", len(entities))
        return entities
