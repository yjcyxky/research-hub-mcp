#!/usr/bin/env python3
"""
Frontiers PDF Downloader Plugin

Downloads PDF files from frontiersin.org URLs.
Frontiers is an open access publisher, so downloads are typically direct HTTP.

Usage:
    from plugins.frontiers_pdf_downloader import FrontiersPDFDownloader

    async with FrontiersPDFDownloader() as downloader:
        result = await downloader.download(
            url="https://www.frontiersin.org/articles/10.3389/xxx.xxx.xxx/pdf",
            output_dir="./downloads"
        )

CLI Usage:
    python -m plugins.frontiers_pdf_downloader <url> [--output-dir ./downloads] [--headless]
"""
...
class FrontiersPDFDownloader(BasePlugin):
    """
    Downloads PDF files from Frontiers.
    ...
    # Frontiers DOI prefixes
    SUPPORTED_PREFIXES = ("10.3389",)
    ...
    def is_supported_url(cls, url: str) -> bool:
        """Check if the URL is a supported Frontiers PDF URL."""
        return cls.SUPPORTED_URL_PATTERN.match(url) is not None

    def build_download_url(self, doi: str) -> Optional[str]:
        """
        Build a Frontiers PDF URL from DOI.

        Args:
            doi: Normalized DOI string (e.g., "10.3389/fxxx.xxx.xxx")
        ...
        normalized_doi = self.normalize_doi(doi)
        return f"https://www.frontiersin.org/articles/{normalized_doi}/pdf"
    ...
    async def _download_direct_http(
        ...
        """
        Attempt direct HTTP download (Frontiers is open access).
    ...
    async def download(
        ...
        """
        Download a PDF from Frontiers.

        Args:
            url: The Frontiers PDF URL
        ...
        if not self.is_supported_url(url):
            return DownloadResult(
                success=False,
                error="Unsupported URL format. Expected: https://www.frontiersin.org/articles/.../pdf",
                doi=doi,
                publisher=self.publisher,
            )

        normalized_doi = self.normalize_doi(doi or "frontiers_download")
        ...
        if pdf_content and pdf_content[:4] == b"%PDF":
            ...
            return DownloadResult(
                success=False,
                error="Failed to download PDF from Frontiers.",
                doi=normalized_doi if doi else None,
                publisher=self.publisher,
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download PDF from Frontiers",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    %(prog)s https://www.frontiersin.org/articles/10.3389/xxx.xxx.xxx/pdf
    %(prog)s https://www.frontiersin.org/articles/10.3389/xxx.xxx.xxx/pdf --output-dir ./papers
        """,
    )
    parser.add_argument("url", help="Frontiers PDF URL to download")
    parser.add_argument(
        "--output-dir",
        "-o",
        default=".",
    ...
    if not FrontiersPDFDownloader.is_supported_url(args.url):
        print("Error: Unsupported URL format")
        print("Expected: https://www.frontiersin.org/articles/.../pdf")
        return 1

    print(f"Downloading: {args.url}")
    print(f"Output directory: {args.output_dir}")

    async with FrontiersPDFDownloader(headless=args.headless) as downloader:
        result = await downloader.download(
            url=args.url,
