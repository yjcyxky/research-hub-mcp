#!/usr/bin/env python3
"""
PLOS PDF Downloader Plugin

Downloads PDF files from journals.plos.org URLs.
PLOS is an open access publisher, so downloads are typically direct HTTP.

Usage:
    from plugins.plos_pdf_downloader import PLOSPDFDownloader

    async with PLOSPDFDownloader() as downloader:
        result = await downloader.download(
            url="https://journals.plos.org/plosone/article/file?id=10.1371/xxx&type=printable",
            output_dir="./downloads"
        )

CLI Usage:
    python -m plugins.plos_pdf_downloader <url> [--output-dir ./downloads] [--headless]
"""
...
class PLOSPDFDownloader(BasePlugin):
    """
    Downloads PDF files from PLOS (Public Library of Science).
    ...
    SUPPORTED_URL_PATTERN = re.compile(
        r"^https?://journals\.plos\.org/[^/]+/article/file\?id=([^&]+)&type=printable$"
    )

    # PLOS DOI prefixes
    SUPPORTED_PREFIXES = ("10.1371",)
    ...
    def is_supported_url(cls, url: str) -> bool:
        """Check if the URL is a supported PLOS PDF URL."""
        return cls.SUPPORTED_URL_PATTERN.match(url) is not None

    def build_download_url(self, doi: str) -> Optional[str]:
        """
        Build a PLOS PDF URL from DOI.

        Args:
            doi: Normalized DOI string (e.g., "10.1371/journal.pone.0123456")
        ...
        normalized_doi = self.normalize_doi(doi)
        return f"https://journals.plos.org/plosone/article/file?id={normalized_doi}&type=printable"
    ...
    async def _download_direct_http(
        ...
        """
        Attempt direct HTTP download (PLOS is open access).
    ...
    async def download(
        ...
        if not self.is_supported_url(url):
            return DownloadResult(
                success=False,
                error="Unsupported URL format. Expected: https://journals.plos.org/.../article/file?id=...&type=printable",
                doi=doi,
                publisher=self.publisher,
            )

        normalized_doi = self.normalize_doi(doi or "plos_download")
        ...
        if pdf_content and pdf_content[:4] == b"%PDF":
            ...
            return DownloadResult(
                success=False,
                error="Failed to download PDF from PLOS.",
                doi=normalized_doi if doi else None,
                publisher=self.publisher,
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download PDF from PLOS",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    %(prog)s "https://journals.plos.org/plosone/article/file?id=10.1371/journal.pone.0123456&type=printable"
    %(prog)s "https://journals.plos.org/plosone/article/file?id=10.1371/journal.pone.0123456&type=printable" --output-dir ./papers
        """,
    )
    parser.add_argument("url", help="PLOS PDF URL to download")
    parser.add_argument(
        "--output-dir",
        "-o",
        default=".",
    ...
    if not PLOSPDFDownloader.is_supported_url(args.url):
        print("Error: Unsupported URL format")
        print("Expected: https://journals.plos.org/.../article/file?id=...&type=printable")
        return 1

    print(f"Downloading: {args.url}")
    print(f"Output directory: {args.output_dir}")

    async with PLOSPDFDownloader(headless=args.headless) as downloader:
        result = await downloader.download(
            url=args.url,
